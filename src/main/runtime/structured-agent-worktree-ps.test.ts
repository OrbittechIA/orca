...
vi.mock('@/runtime/local-runtime-capabilities', () => ({
  // What a real host advertises: structured sessions exist, and the scoped Work Item Start
  // route exists. Inlined because `vi.mock` is hoisted above any top-level binding.
...
describe('structured agent worktree.ps projection', () => {
  it('runs Work Item Start through one durable native writer and authoritative worktree.ps', async () => {
    ...
    // The scoped admission resolves through the folder-aware path, not `showManagedWorktree`:
    // that resolver knows repo-derived worktrees only and refused every folder workspace.
    vi.spyOn(runtime, 'resolveStructuredAgentSessionCreateWorktreeTarget').mockResolvedValue(
      structuredAgentSessionCreateWorktreeTarget({
        id: TEST_WORKTREE_ID,
        path: `/workspaces/${TEST_WORKTREE_ID}`,
