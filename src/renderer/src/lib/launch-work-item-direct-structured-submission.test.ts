...
// A real host advertises both: `agent-session.structured.v1` says it has structured
// sessions at all, `agent-session.work-item-start.v1` says it has the scoped route this
// launch needs. Declaring only the first is an older host, which is a separate case.
// Inlined because `vi.mock` is hoisted above any top-level binding.
...
describe('launchWorkItemDirect structured submission', () => {
  beforeEach(() => {
    ...
    mocks.startStructuredAgentLaunch.mockReturnValue({
      ...
      releaseCallerAfterUnknownOutcome: vi.fn(() => false)
    })
  ...
  it('fails closed instead of launching a configured Codex wrapper', async () => {
    ...
    // A configured TUI wrapper is knowable before `git worktree add`, so the strict Start
    // refuses before creating rather than leaving a revealed workspace with no writer.
  ...
  it('fails closed on SSH instead of inferring readiness from remote PTY output', async () => {
    ...
    expect(mocks.startStructuredAgentLaunch).not.toHaveBeenCalled()
    expect(pasteDraftWhenAgentReady).not.toHaveBeenCalled()
    // It used to create and reveal the workspace first and fail closed afterwards, which on
    // a remote host meant a worktree created on the far side and then abandoned — a
    // workspace that looks started and has no writer. A remote execution host is knowable
    // before `git worktree add`, so the refusal happens before it.
    expect(mocks.store.createWorktree).not.toHaveBeenCalled()
    expect(mocks.activateAndRevealWorktree).not.toHaveBeenCalled()
  })
})
